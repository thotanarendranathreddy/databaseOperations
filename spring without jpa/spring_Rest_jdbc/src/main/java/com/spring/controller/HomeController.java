...
public class HomeController {
	...
	public List<Customer> getAll(@PathVariable String name) {
	...
	@RequestMapping(value = "/getEmployee/{id}/{value}", method = RequestMethod.GET,headers="Accept=application/json")
	public List<Customer> getEmployee(@PathVariable int id) {
				logger.info("This is used to search employee details");
				logger.info(info);
		int contactId=id;
		List<Customer> list;
		list=contactDAO.get(contactId);
		return list;
	}
	...
	public String create(@RequestBody String employee) {
		...
		Boolean status1= contactDAO.create(empObj.getInt("id"),empObj.getString("name"),empObj.getInt(sal),empObj.getString("dept"));
		logger.info(status1);
					if(!status1){
